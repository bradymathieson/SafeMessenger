...
def add_user(username, ip, port):
    ...
    req = requests.post(BASE_URL+"/add_user", data=json.dumps(post_data), headers=HEADERS)
...
def remove_user(username, ip, port):
    ...
    req = requests.post(BASE_URL+"/remove_user", data=json.dumps(post_data), headers=HEADERS)
